...
$userSerch = $user->getUserID($_GET["user"],$db);
if ($userSerch == $_SESSION["userID"]){
    header("Location: profile.php");
}

if($user->getFriendship($_SESSION["userID"],$userSerch)){
    $friendship = true;
}else{
    if($user->getFriendshipAsked($_SESSION["userID"],$user->getUserID($_GET["user"],$db))){
        $friendshipAsked = "true";
    }else{$friendshipAsked = "false";}
}
$status = $user->checkDown($_GET["user"]);
if ($status === "false"){
    $status = false;
}
?>
                        ...
                        <div class="col-lg-12">
                            <div class="card w-100 border-0 p-0 bg-white shadow-xss rounded-xxl">
                                ...
                                <div class="card-body p-0 position-relative">
                                    <figure class="avatar position-absolute w100 z-index-1" style="top:-40px; left: 30px;"><img style="height: 100px;width: 100px; object-fit: cover;" src="
                                    <?php
                                        $searchedUserName = $_GET["user"];
                                        $searchedUserId = $user->getUserID($searchedUserName,$db);
                                        $user->getUserImage($searchedUserId,$db);

                                    ?>
                                    " alt="image" class="float-right p-1 bg-white rounded-circle w-100"></figure>

                                    <h4 class="fw-700 font-sm mt-2 mb-lg-5 mb-4 pl-15"><?php $user->getName($searchedUserId,$db);if(!$status){?>
                                            <button class="d-none d-lg-block bg-greylight btn-round-lg ms-2 rounded-3 text-grey-700" style="border: none;float: right;margin-right: 30px; margin-top: -13px;scale: 1; background-color: lightgreen !important;" onclick="acctivateAccount()"> <i class="feather-check-circle font-md"></i></button>
                                            <span style="float: right;margin-right: 10px; color: dimgray">Suspendida</span>

                                        <?php }?><span class="fw-500 font-xssss text-grey-500 mt-1 mb-3 d-block"><?php echo $_GET["user"]; ?></span></h4>
                                    <div id="friendADD" class="d-flex align-items-center justify-content-center position-absolute-md right-15 top-0 me-2">
                                        ...
                                        <?php
                                        //Hay amistad
                                            if ($friendship and $status){
                                            ?>
                                            <button style="border: none" class="d-none d-lg-block bg-success p-3 z-index-1 rounded-3 text-white font-xsssss text-uppercase fw-700 ls-3"  id="friendship" onmouseover="getRed(this)" onmouseleave="getGreen(this)" onclick="deleteFriend()"> Amigos</button>
                                                <script type="application/javascript">

                                                    function deleteFriend(){
                                                        var idFriend = <?php echo $searchedUserId; ?>;
                                                        var deleteF = true;
                                                        $.ajax({
                                                            type: "POST",
                                                            url: "credentials/friendshipCreation.php",
                                                            data: {deleteFriend : deleteF, FriendUser : idFriend},
                                                            success: function (resolve) {
                                                                alertify.error('Amistad eliminada');
                                                                delay(1500).then(() => relocation());

                                                            }
                                                        });
                                                    }
                                                    function getRed(object){
                                                        object.classList.remove("bg-success");
                                                        object.classList.add("bg-danger");
                                                        object.innerHTML = "Borrar amistad";

                                                    }
                                                    function getGreen(object){
                                                        object.classList.remove("bg-danger");
                                                        object.classList.add("bg-success");
                                                        object.innerHTML = "Amigos";
                                                    }
                                                </script>
                                            <?php
                                            //else se ha pedido amistad pero no se ha aceptado
                                            }else{
                                                if($friendshipAsked =="true" and $status){
                                                ?>

                                                <button style="border: none" class="d-none d-lg-block bg-warning p-3 z-index-1 rounded-3 text-white font-xsssss text-uppercase fw-700 ls-3"  id="friendship" onmouseover="getRed(this)" onmouseleave="getGreen(this)" onclick="deleteFriend()">Enviada</button>
                                                <script type="application/javascript">
                                                    function deleteFriend(){
                                                        var idFriend = <?php echo $searchedUserId; ?>;
                                                        var deleteF = true;
                                                        $.ajax({
                                                            ...
                                                            data: {deleteFriend : deleteF, FriendUser : idFriend},
                                                            success: function (resolve) {
                                                                    alertify.error('Petición eliminada');
                                                                    delay(1500).then(() => relocation());
                                                    ...
                                                    function getRed(object){
                                                        ...
                                                        object.classList.add("bg-danger");
                                                        object.innerHTML = "Borrar";

                                                    }

                                                    function getGreen(object){
                                                        object.classList.remove("bg-danger");
                                                        object.classList.add("bg-warning");
                                                        object.innerHTML = "Enviada";
                                                    }
                                                ...
                                                <?php
                                                //else no se ha pedido amistad

                                                }else{ if ($status){
                                                ?>
                                                <div id="altaAmistad">
                                                    <button style="border: none" class="d-none d-lg-block bg-warning p-3 z-index-1 rounded-3 text-white font-xsssss text-uppercase fw-700 ls-3"  id="friendship" onclick="getRed(this)">Solicitar amistad</button>
                                                </div>
                                                <script type="application/javascript">
                                                    var click = 0;
                                                    function getRed(object){
                                                        var userAskID = "<?php echo $_SESSION["userID"]?>";
                                                        var userAskedID = "<?php echo $user->getUserID($_GET["user"],$db)?>";
                                                        if (click = 0){
                                                            $.ajax({
                                                                type: "POST",
                                                                url: "credentials/friendshipCreation.php",
                                                                data: {userAskID : userAskID, userAskedID : userAskedID},
                                                                success: function (resolve) {
                                                                    if (resolve == "true"){
                                                                        //alertify.success('Petición enviada');
                                                                        object.innerHTML = "Anular amistad";
                                        ...
                                        <script>
                                            ...
                                            function relocation(){
                                                window.location.href = "user-page.php?user=<?php echo $_GET["user"]?>";

                                            }
                                            function acctivateAccount(){
                                                var user = <?php echo $user->getUserID($_GET["user"],$db);?>;
                                                $.ajax({
                                                    type: "POST",
                                                    url: "credentials/activateAcc.php",
                                                    data: {idUser: user},
                                                    success: function (resolve) {
                                                        alertify.success("Cuenta reactivada");
                                                        delay(1500).then(() => relocation());
                                                    }
                                                });
                                        ...
                                        if ($friendship == true and $status){
                                        ?>

                                        <script>

                                            function mensaje(){
                                                window.location("default-message.php?user=<?php echo $userSerch;?>");
                                            }

                                        </script>
                                            <a href="default-message.php?user=<?php echo $userSerch?>"><button class="d-none d-lg-block bg-greylight btn-round-lg ms-2 rounded-3 text-grey-700" style="border: none"> <i class="feather-mail font-md"></i></button> </a>

                                        <?php }?>
                                        <?php if (isset($_SESSION["admin"]) and $status){?>
                                                <script>
                                                    function suspendAccount(){
                                                        var user = <?php echo $user->getUserID($_GET["user"],$db);?>;
                                                        $.ajax({
                                                            type: "POST",
                                                            url: "credentials/suspendAcc.php",
                                                            data: {idUser: user},
                                                            success: function (resolve) {
                                            ...
                                            <button class="d-none d-lg-block bg-greylight btn-round-lg ms-2 rounded-3 text-grey-700" style="border: none; background-color: lightcoral !important;" onclick="suspendAccount()"> <i class="feather-x-circle font-md"></i></button>
                                        <?php }?>
                                    </div>
                                </div>
                                <script>
                                ...
                                <div class="card-body d-block w-100 shadow-none mb-0 p-0 border-top-xs">
                                    <ul class="nav nav-tabs h55 d-flex product-info-tab border-bottom-0 ps-4" id="pills-tab" role="tablist">
                                        ...
                                        <li class="active list-inline-item me-5"><a class="fw-700 font-xssss text-grey-500 pt-3 pb-3 ls-1 d-inline-block" href="<?php if ($friendship and $status){ echo "amigos.php?user=".$userSerch;}?>" data-toggle="tab" onclick="amigos()"><?php if ($friendship and $status){echo $user->getNumFriends($userSerch);} ?> Amigos</a></li>
                                    </ul>
                                </div>
                                <?php
                                }
                            ...
                            <div class="card w-100 shadow-xss rounded-xxl border-0 mb-3">
                                ...
                                <div id="formacion" hidden>
                                    <div class="card-body d-block p-4">
                                        <h4 class="fw-700 mb-3 font-xsss text-grey-900">Formacion
                                            <?php if (isset($_SESSION["admin"])){?>
                                            <a href="formacion-informationUser.php?user=<?php echo $user->getUserID($_GET["user"],$db);?>"><button class="d-none d-lg-block bg-greylight btn-round-lg ms-2 rounded-3 text-grey-700" style="border: none; scale: 0.8; float: right; margin-top: -6%" > <i class="feather-edit font-md"></i></button></a>
                                            <?php
                                            }
                                            ?>
                                        </h4>
                                ...
                                <div id="experiencia" hidden>
                                    <div class="card-body d-block p-4">
                                        <h4 class="fw-700 mb-3 font-xsss text-grey-900">Experiencia
                                            <?php if (isset($_SESSION["admin"])){?>
                                                <a href="laboral-informationUser.php?user=<?php echo $user->getUserID($_GET["user"],$db);?>"><button class="d-none d-lg-block bg-greylight btn-round-lg ms-2 rounded-3 text-grey-700" style="border: none; scale: 0.8; float: right; margin-top: -6%" > <i class="feather-edit font-md"></i></button></a>
                                                <?php
                                            }
                                            ?></h4>
                                    </div>
                                    ...
                                    ?>
                                        <div class="card-body d-flex pt-0">
                                            <h4 class="fw-700 text-grey-900 font-xssss mt-1"><?php if ($lab){ echo $user->getEmpresa($laboral[0]["4"]); ?> - <?php echo $user->getPuesto($lab["5"]); }?></h4>
                                            <div style="margin-left: 10px;"></div>
